...
| `ResolveGameSymbol(moduleBase, name, expectedKind, &address)` | 解析为 `moduleBase + rva`；`expectedKind` 必须为 FUNCTION / GLOBAL / PATCH，kind 不符时返回 `MH_GAMESYMBOL_KIND_MISMATCH`。 |
