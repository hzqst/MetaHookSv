...
- 大多数定位在镜像映像 `DllInfo`（优先使用 `g_hMirrorEngine`）中完成，再经 `ConvertDllInfoSpace` 按 RVA 映射回实际加载映像 `RealDllInfo`；转换实现见 `src/metahook.cpp:1269-1280`，所有定位例程的调度见 `src/metahook.cpp:2354-2366`。
- 定位失败会通过 `MH_SysError` 中止加载；这意味着下表的强制项均有运行时门禁。各变量还会在 `MH_ResetAllVars` 中清零（`src/metahook.cpp:1210-1242`）。
...
| `pfnHUDInit`（临时局部；client `HUD_Init` 回调指针槽） | `src/metahook.cpp:1523` | `src/metahook.cpp:1504-1576`：从 `"cl_righthand"` 的 `.text` 交叉引用反向匹配 `A1 <imm32>; test eax`；`A1` 的立即数是该私有全局槽地址。 | `src/metahook.cpp:1551-1554` 仅作为排除条件，防止将 HUD 初始化回调槽误判为 `g_phClientModule`。该地址不会长期保存。 |
| `g_phClientModule`（`HMODULE *`，游戏的 client DLL 模块句柄槽） | `src/metahook.cpp:115` | `src/metahook.cpp:1504-1596`：沿同一 `"cl_righthand"` 锚点在 `HUD_Init` 邻域反汇编，提取位于 `.data` 的绝对内存操作数并排除 `pfnHUDInit`；fallback 匹配 `A1 <imm32>; push; call`。 | `src/metahook.cpp:1171-1180` 控制 client 镜像加载；`src/metahook.cpp:2146-2159` 约束 `FreeBlob` 签名；`src/metahook.cpp:3292-3295` 对外返回模块句柄。 |
...
| `g_pVideoMode`（视频模式对象指针槽） | `src/metahook.cpp:101` | `src/metahook.cpp:1643-1792`：以 `"-fullscreen"` 为锚点（非 SvEngine 先尝试 `"-gl"`）；在后续 `0x400` 字节反汇编，匹配对 `.data` 绝对地址写零/寄存器的 `mov`，并要求后 15 条指令内出现 `ret`。 | `src/metahook.cpp:3528-3592` 解引用为 `IVideoMode` / `IVideoMode_HL25`，读取、返回及保存视频模式。 |
...
| 本地符号 | 声明位置 | 定位代码范围与原理 | 后续使用 |
| --- | --- | --- | --- |
| `g_StudioInterfaceCall`（`ClientDLL_CheckStudioInterface` 中的 studio 接口调用指令地址） | `src/metahook.cpp:109` | `src/metahook.cpp:2175-2273`：复用 studio 错误字符串锚点；直接调用模式保存 `pStudioInterfaceCall_VA`，间接调用模式保存 `pStudioInterfaceIndirectCall_VA + 9`。这是一条待改写的 call-site，不是可独立调用的函数入口。 | `src/metahook.cpp:2375-2389` 用 `MH_WriteMemory` 覆写为跳转 `CheckStudioInterfaceTrampoline` 的指令序列。 |
