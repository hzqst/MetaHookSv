...
3. 眉毛部分的像素的alpha通道应当小于255, 可以利用 `replacetexture` 将基础贴图替换为支持Alpha透明通道的 PNG / BC7-DDS 贴图来完成：

![](/img/10.png)

* 当包含眉毛的贴图附带了高光贴图时，高光贴图的Alpha通道会被混合至基础色贴图。这在 `replacetexture` 使用了不支持Alpha透明通道的HDR格式贴图时，可以用来代替基础色贴图的Alpha通道。

当以上条件全部满足时，你就可以透过头发看见眉毛：

![](/img/9.png)

* 注意：alpha值越低，其在头发中的可见度就越高。
