...
* 注意： [BugFixedHL](https://github.com/tmp64/BugfixedHL-Rebased) 使用了不同的VGUI2对象布局, 因此无法兼容 `VGUI2Extension.dll` 。
...
高DPI缩放支持仅在你的系统DPI缩放比例 > 100% 并且你使用的不是HL周年更新版本的引擎时才会默认启用，其他情况下默认不启用。对于HL25引擎，默认启用高DPI缩放支持。
