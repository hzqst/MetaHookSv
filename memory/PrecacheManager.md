...
## Overview
`PrecacheManager` is a MetaHookSv utility plugin. After client initialization, it registers the `fs_dump_precaches` console command to export the current map's precached resources to `[ModDirectory]\\maps\\[mapname].dump.res`. During engine loading, the plugin resolves the runtime resource-list head `cl_resourcesonhand` directly from gamedata.

## Responsibilities
- Initializes the plugin runtime environment during `IPluginsV4::LoadEngine` (file-system interfaces, engine type/version, and the real engine base).
- Calls `Engine_FillAddress` to resolve the `cl_resourcesonhand` address from the gamedata GLOBAL record, establishing the resource-iteration entry point.
...
2. **Address-resolution layer** (`privatehook.cpp`): resolves the gamedata GLOBAL `cl_resourcesonhand` once via `ResolveGameSymbol(..., MH_GAMESYMBOL_KIND_GLOBAL, ...)` against the real engine base and assigns the returned sentinel address directly to the plugin variable; failure is fatal.
...
flowchart TD
    ...
    C --> D[ResolveGameSymbol: cl_resourcesonhand GLOBAL]
...
## Dependencies
- **MetaHook API**: `ResolveGameSymbol`, `GetModuleCRC64`, `GetGameSymbolStatusString`, `GetEngineType`, `GetEngineBuildnum`, `GetEngineBase`, and `SysError`.
...
- Resource iteration depends on `cl_resourcesonhand` being resolved successfully; a missing gamedata record or resolve failure triggers `Sys_Error` with symbol / buildnum / CRC64 / status diagnostics at load time instead of leaving the pointer null.
