...
- **Required vs optional resolution (2026-09-12).** `GamedataResolvePtr(moduleBase, name, kind, required)` / `GamedataResolveScalar(moduleBase, name, required)` replaced the always-fatal helpers; `required=false` returns `nullptr` / `0` instead of `Sys_Error`. Invariant symbols (engine render/view + Studio functions, engine globals, `size_of_frame`, `g_iUser1` / `g_iUser2`, the Sven Co-op and the CS/CZ globals) pass `required=true` and stay fatal. The three client `GameStudioRenderer_*` virtuals pass `required=false`: the install/uninstall paths already null-check them, so a client without the client studio renderer keeps the engine-side hooks. `scripts/validate-gamedata.py` mirrors the split with `BULLETPHYSICS_CLIENT_OPTIONAL_VFUNCS` — kind is checked when the record is present, presence is never required. The client `g_pGameStudioRenderer` global is no longer resolved or referenced at all: it lost its only consumer (the vtable base for by-index virtual resolution) when virtuals moved to by-name `virtualFunction` records, so it was dropped from the plugin and from the validator's required list.
...
- Glue consumed only inside `exportfuncs.cpp`/`privatehook.cpp` (no external plugin consumers): `cl_viewentity`, `cl_frames`, `size_of_frame`, `mod_known`, `mod_numknown`, `g_iUser2`, `g_pitchdrift`, `pstudiohdr`, `g_bRenderingPortals_SCClient`.
...
| `GameStudioRenderer_StudioDrawModel` / `_StudioDrawPlayer` / `_StudioSetupBones` | virtualFunction | `__fastcall` fn ptrs | optional (`required=false`); hooked when present (client Studio path), otherwise engine-side hooks only |
