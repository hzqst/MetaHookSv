...
The engine VGUI2 surface (`EngineSurface007` → `IEngineSurface`/`IEngineSurface_HL25`, and `VGUI_Surface026`) is obtained from `g_pMetaHookAPI->GetEngineFactory()`; the plugin then hooks methods by hardcoded vtable index with `VFTHook(obj, 0, index, wrapper, &original)`. `EngineSurface_FillAddress` resolves the function pointers only to validate indices and to drive one disasm root (`pushMakeCurrent`). `EngineSurface_InstallHooks` installs 19 engine-surface hooks (+`VGUI_Surface026::DrawSetTexture` at index 27); every wrapper replaces the engine method and never calls the saved original, except `BaseUISurface_DrawSetTexture`.
