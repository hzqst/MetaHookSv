...
| `texture_extension_number_mov_site_*` PATCH records | 5-byte rewrite to `call GL_RedirectedGenTexture` | `R_RedirectEngineLegacyOpenGLTextureAllocation` probes seven complete names and resolves published PATCH addresses against the real engine image; CoF publishes two sites, legacy HL five more. No-op when `g_bHasOfficialGLTexAllocSupport`. | Legacy texture allocation through the plugin `GL_GenTexture`. |
...
- **Correct approach:** Keep the official-allocation guard, iterate seven complete `texture_extension_number_mov_site_*` names, distinguish `SYMBOL_NOT_FOUND` from other query errors, resolve each present PATCH against the real engine module, and write the existing five-byte relative `call GL_RedirectedGenTexture`. The old mirror scan and the `allocated_textures` pointer are gone. The capability probe still uses the `texture_extension_number` GLOBAL's presence.
