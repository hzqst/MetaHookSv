...
- `GL_Init`, `GL_Bind`, `GL_SelectTexture`, `GL_Set2D`, `GL_Finish2D`, `GL_BeginRendering`, `GL_EndRendering`, `Host_IsSinglePlayerGame` — flat gamedata assigns; `GL_Init` also resolves `gl_extensions` and `GL_Bind` also `currenttexture`. Their standalone locators were removed 2026-09-14 and the dispatch resolved them inline until 2026-09-24, when each got a named locator back (see the last section).
...
`Engine_FillAddress` copies SDL2 exports (`GetProcAddress(GetModuleHandle("SDL2.dll"), …)`, excluded) and the public `gEngfuncs.pTriAPI->*` pointers, resolves `SvEngine_glewInit` from the engine export `_glewInit@0` (gated on the `SCEngineClient002` factory), then calls 90 locators in a fixed order. Order is observable in the rare case two locators fill the same field. Since 2026-09-24 the dispatch body contains no raw gamedata assign — every resolve lives in a locator (see the last section). Highlights:
...
| `gPrivateFuncs.R_ForceCVars` / `R_CheckVariables` / `R_AnimateLight` | `void (*)(qboolean mp)` / `void (*)(void)` / `void (*)(void)` | `Engine_FillAddress_R_ForceCVars`: three `GamedataResolvePtr` FUNCTION calls (SVEngine pattern / `R_SETUPFRAME_CALL_SIG` history removed 2026-09-22; inline in the dispatch 2026-09-22..09-24). | `Install_InlineHook(R_ForceCVars)`; `R_CheckVariables` / `R_AnimateLight` wrap the original, `R_ForceCVars` is a full reimplementation and never calls its trampoline. |
...
| `pmainwindow` | `void**` | `GamedataResolvePtr` GLOBAL `pmainwindow` in `Engine_FillAddress_EngineSurface_pushMakeCurrent` (see EngineSurface); the `DisasmRanges` heuristic was dropped 2026-09-26. | Main window handle. |
...
| `gltextures` / `gltextures_SvEngine` / `maxgltextures_SvEngine` / `peakgltextures_SvEngine` / `numgltextures` / `gHostSpawnCount` | texture-array bookkeeping | `Engine_FillAddress_GL_LoadTexture2` resolves the engine globals through `GamedataResolvePtr` (and SvEngine's `realloc` function); the unused `allocated_textures` pointer was removed on 2026-09-24. | Texture enumeration/unload/growth. |
...
| `g_iFogColor_SCClient` / `g_iStartDist_SCClient` / `g_iEndDist_SCClient` | `float*` | `Client_FillAddress_FogParams` resolves client GLOBAL records `g_iFogColor`, `g_iStartDist`, and `g_iEndDist` through `GamedataResolvePtr` on the real client module. | Sven fog params. |
...
| `texture_extension_number_mov_site_*` PATCH records | Replace a complete `MOV EAX, [mem]` instruction with `call GL_RedirectedGenTexture`, padding any bytes beyond the five-byte call with NOPs | `R_RedirectEngineLegacyOpenGLTextureAllocation` probes seven complete names and resolves published PATCH addresses against the real engine image. `DisasmSingleInstruction` requires a matching instruction of at least five bytes before writing; CoF publishes two sites, legacy HL five more. No-op when `g_bHasOfficialGLTexAllocSupport`. | Legacy texture allocation through the plugin `GL_GenTexture`. |
...
The engine VGUI2 surface (`EngineSurface007` → `IEngineSurface`/`IEngineSurface_HL25`, and `VGUI_Surface026`) is obtained from `g_pMetaHookAPI->GetEngineFactory()`; the plugin then hooks methods by hardcoded vtable index with `VFTHook(obj, 0, index, wrapper, &original)`. `EngineSurface_FillAddress` resolves the function pointers only to validate indices; its last disasm root (`pushMakeCurrent`) was removed on 2026-09-26 when the three globals it walked for moved to gamedata, so this file no longer includes `<capstone.h>`. `EngineSurface_InstallHooks` installs 19 engine-surface hooks (+`VGUI_Surface026::DrawSetTexture` at index 27); every wrapper replaces the engine method and never calls the saved original, except `BaseUISurface_DrawSetTexture`.

| Local symbol / inferred engine symbol | Signature of field | Resolution mechanism | Subsequent use |
| --- | --- | --- | --- |
| ~~`gPrivateFuncs.enginesurface_pushMakeCurrent`~~ | `void (__fastcall*)(void*, int, int* insets, int* absExtents, int* clipRect, bool translateToScreenSpace)` | ~~`GetVFunctionFromVFTable(vftable, index=1, …)`; also the scan root for `pmainwindow`/`g_bScissor`/`g_ScissorRect`.~~ Deleted 2026-09-26: once the disasm root went away, the two `GetVFunctionFromVFTable` assignments were its only remaining occurrences. `index_enginesurface_pushMakeCurrent` stays — the VFTHook needs it. | VFTHook index 1; wrapper replaces engine method. |
| `enginesurface_popMakeCurrent` / `drawFilledRect` / `drawOutlinedRect` / `drawLine` / `drawPolyLine` / `drawTexturedPolygon` / `drawSetTextureRGBA` / `drawSetTexture` / `drawTexturedRect` / `drawTexturedRectAdd` / `createNewTextureID` / `drawPrintCharAdd` / `drawSetTextureFile` / `drawGetTextureSize` / `isTextureIDValid` / `drawSetSubTextureRGBA` / `drawFlushText` / `drawSetTextureBGRA` / `drawUpdateRegionTextureBGRA` | surface virtuals | Hardcoded `index_enginesurface_*`; HL25 uses a higher-index layout than GoldSrc/SvEngine. `drawTexturedRectAdd` is hooked on HL25 only; `drawTexturedPolygon` is hooked by index but its field is never populated; `drawFlushText` is hooked by index, but its `gPrivateFuncs.enginesurface_drawFlushText` field was deleted 2026-09-26 (it was only ever the disasm root for `g_iVertexBufferEntriesUsed`/`g_VertexBuffer`). | VFTHook per index; wrappers reimplement via `R_Draw*`. |
...
| `g_iVertexBufferEntriesUsed` / `g_VertexBuffer` | `int*` / `EngineSurfaceVertexBuffer_t (*)[256]` | `GamedataResolvePtr(RealDllInfo.ImageBase, …, GLOBAL)` since 2026-09-26 (both names published 11/11). Previously `Engine_FillAddress_EngineSurface_drawFlushText`: `DisasmRanges` mirror body `+0x150`; `CMP [abs],imm` or `MOV reg,[abs]`+`CMP reg,imm`; then first `PUSH imm32(.data)` = `g_VertexBuffer`. | Engine vertex-buffer flush. |
| `g_bScissor` / `g_ScissorRect` | `bool*` / `RECT*` | `GamedataResolvePtr(RealDllInfo.ImageBase, …, GLOBAL)` since 2026-09-26 (both names published 11/11). Previously `Engine_FillAddress_EngineSurface_pushMakeCurrent`: `DisasmRanges` mirror body `+0x500`; `MOV [.data],1` = `g_bScissor`; up to 4 successive `MOV [.data],reg`, `qsort`, lowest = `g_ScissorRect`. | Scissor clipping. |
| `pmainwindow` | `void**` | `GamedataResolvePtr(RealDllInfo.ImageBase, …, GLOBAL)` since 2026-09-26 (published 11/11). Previously the same function: `MOV reg,[.data]` within first 35 instructions, validated by `MOV reg,[candReg]`/`PUSH [candReg]` within `+6`. | `Sys_GetMainWindow()`. |
...
- **`g_bHasOfficialGLTexAllocSupport`** is a capability flag, not an address: absence of the `texture_extension_number` gamedata GLOBAL means official allocation support. It gates the legacy texture-allocation redirect. The old `g_bHasOfficialFBOSupport` flag was removed.
...
## Legacy OpenGL texture allocation redirect via gamedata (2026-09-24)

- **Trigger / constraint:** `R_RedirectEngineLegacyOpenGLTextureAllocation` still scanned `mov eax, [texture_extension_number]` instructions and disassembled nearby code to decide which reads allocated textures. The supported gamedata snapshots already publish those instruction addresses as engine PATCH records.
- **Correct approach:** Keep the official-allocation guard, iterate seven complete `texture_extension_number_mov_site_*` names, distinguish `SYMBOL_NOT_FOUND` from other query errors, and resolve each present PATCH against the real engine module. Since the 2026-09-24 follow-up, decode one instruction at each address and require `MOV EAX, [mem]` with length ≥5 before writing the five-byte relative `call GL_RedirectedGenTexture`; pad any remaining instruction bytes with NOPs. The old mirror scan and the `allocated_textures` pointer are gone. The capability probe still uses the `texture_extension_number` GLOBAL's presence.
- **Coverage:** `cof-5936` publishes `GL_LoadFilterTexture` and `R_InitParticleTexture`; `hl-3248`, `hl-3266`, `hl-3329`, `hl-3647`, and `hl-4554` also publish `GL_BuildLightmaps`, `GL_LoadTexture2`, `LoadTransPic_bind`, `LoadTransPic_increment`, and `R_Init_playertextures`. The Renderer release gate requires the corresponding PATCH records by family.
- **Verification:** `python -m unittest discover -s scripts/tests -p test_gamedata_contract.py` passed 70 tests; `python scripts/validate-gamedata.py Build/svencoop/metahook/gamedata` passed 21 snapshots / five engine families; `Renderer.vcxproj` Debug|Win32 MSBuild completed successfully. In-game validation was not run.
- **Scope:** This changes the legacy allocation call-site lookup only. GameData still requires exact names; it does not enumerate symbols by prefix.

## Sven client fog globals via gamedata (2026-09-24)

- **Trigger / root cause:** `Client_FillAddress_FogParams` still searched the client image for `GL_LINEAR` / `GL_FOG` bytes and inferred three live fog globals from nearby `MOVSS` operands. Both supported Sven Co-op client snapshots publish the final Windows GLOBAL addresses.
- **Correct approach:** In the `SCClientDLL001` branch, resolve `g_iFogColor`, `g_iStartDist`, and `g_iEndDist` from the real client module with `GamedataResolvePtr`; retain the plugin's `_SCClient` pointer names and their existing consumers. Require all three GLOBAL records in the Renderer Sven client gate.
- **Verification:** `python -m unittest discover -s scripts/tests -p test_gamedata_contract.py` passed 71 tests; `python scripts/validate-gamedata.py Build/svencoop/metahook/gamedata` passed 21 snapshots / five engine families; Renderer Debug|Win32 MSBuild completed successfully. In-game fog behavior was not tested.
- **Scope:** Only `Client_FillAddress_FogParams` and its gated symbols changed; other Sven client locators remain as they were.

## Engine locator self-guards removed, inline resolves extracted (2026-09-24)

- **Trigger / goal:** Every `Engine_FillAddress_*` locator still opened with a self-guard (`if (gPrivateFuncs.X) return;`) although `Engine_FillAddress` calls each one exactly once, and the dispatch body still carried 34 groups of raw `GamedataResolvePtr` assigns between the locator calls. The locator layer is now uniform: guard-free locators plus a dispatch that only sequences locator calls.
- **Self-guards removed (25):** `GL_Init`, `GL_SetMode`, `GL_Bind`, `GL_LoadTexture2`, `R_CullBox`, `R_RenderView`, `V_RenderView`, `R_NewMap`, `R_DrawSequentialPoly`, `BuildGammaTable`, `R_DrawParticles`, `Cache_Alloc`, `Draw_DecalTexture`, `GlowBlend`, `Hunk_AllocName`, `DrawStartupGraphic`, `DrawStartupVideo`, `Draw_SpriteFrameHoles`, `Draw_SpriteFrameAdditive`, `Draw_SpriteFrameGeneric`, `Draw_FillRGBABuf`, `Mod_UnloadSpriteTextures`, `Draw_FillRGBA`, `Draw_FillRGBABlend`, `D_FillRect`. Behaviour is unchanged: each locator has exactly one caller, and no locator name is referenced outside the dispatch.
- **Engine-type branches kept — they are load-bearing, not redundant.** They select catalog names the other identities do not publish. Windows-record check over the packaged snapshots: `Draw_SpriteFrame{Holes,Additive,Generic}_SvEngine` / `R_LoadSkyBox_SvEngine` / `r_missingtexture` / `allow_cheats` / `Draw_FillRGBABuf` / `c_model_polys` / `gmodinfo_vertical_fov` exist **only** on svencoop, and `Draw_SpriteFrame*` / `R_LoadSkys` / `r_notexture_mip` / `D_FillRect` / `c_alias_polys` **only** on the other families. `GlowBlend` carries **linux-only** records on svencoop-10257/8948 and hl-10210, so its `SvEngine || HL25` early-out stays as well. `HasOfficialGLTexAllocSupport` and `GL_EndRenderingVars` keep their `IsGameSymbolAvailable` probes and storage fallback.
- **34 locators restored / extracted** (dispatch order): `GL_Shutdown` (FUNCTION + `Sys_ShutdownGame_to_GL_Shutdown_callsite_0` PATCH), `V_FadeAlpha` (+ `cl_sf`), `S_ExtraUpdate`, `GL_SelectTexture`, `R_ForceCVars` (+ `R_CheckVariables`, `R_AnimateLight`), `GL_LoadFilterTexture`, `GL_BuildLightmaps`, `R_TextureAnimation` (+ `rtable`), `GL_Set2D`, `GL_Finish2D`, `GL_BeginRendering`, `GL_EndRendering`, `Mod_PointInLeaf`, `R_DrawTEntitiesOnList`, `CL_DlightVars` (`cl_dlights` + `cl_elights`), `R_GLStudioDrawPoints`, `Host_ClearMemory`, `R_GetSpriteFrame`, `Host_IsSinglePlayerGame`, `Mod_LoadSpriteModel`, `VisEdicts` (`cl_numvisedicts` + `cl_visedicts`), `CL_SimOrgVars` (`cl_simorg`), `CL_ViewEntityVars` (`cl_viewentity`), `CL_ReallocateDynamicData` (`cl_max_edicts` + `cl_entities`), `TempEntsVars` (`gTempEnts`), `ModKnownVars` (`mod_known` + `mod_numknown`), `Mod_LoadStudioModel`, `Mod_LoadBrushModel`, `Mod_LoadModel`, `SetFilterMode`, `SetFilterColor` (`filterColorRed`/`Green`/`Blue`), `SetFilterBrightness`, `PVSNode`, `Draw_Pic`. `Engine_FillAddress_*` count in `gl_hooks.cpp` goes 54 → 88, each defined once and called once; dispatch order, the SDL2 block, the `gEngfuncs.pTriAPI->*` copies and the `_glewInit@0` resolve are untouched. Naming follows the existing convention (locator named for its anchor symbol; historical names reused where the note records them, e.g. `VisEdicts`, `CL_SimOrgVars`, `CL_ViewEntityVars`, `CL_ReallocateDynamicData`, `TempEntsVars`, `ModKnownVars`, `SetFilter*`).
- **Superseded wording:** the "resolved inline in the dispatch" statements elsewhere in this note (the `GL_SelectTexture` / `GL_Set2D` … bullet under Gamedata-resolved symbols, the `R_ForceCVars` and `cl_dlights` / `cl_elights` table rows, dispatch-order item 3, and the `CL_SimOrgVars` / `CL_ViewEntityVars` / `SetFilter*` notes) describe the 2026-09-14..09-22 state; each of those symbols has a named locator again.
- **Verification:** `Renderer.vcxproj` Release|Win32 MSBuild (`MetaHook.sln -target:Plugins\Renderer`) exits 0 and emits no warning from `gl_hooks.cpp`; a `def == call` check over the file reports 88 definitions / 88 dispatch calls, with no name defined twice or called twice. In-game smoke tests were not run.
- **Scope:** `Plugins/Renderer/gl_hooks.cpp` plus this note. `Client_FillAddress_*` self-guards were deliberately left in place, and no symbol name, kind, or release gate changed.

## Legacy texture allocation instruction validation (2026-09-24)

- **Trigger / constraint:** A gamedata PATCH address was previously overwritten with a fixed five-byte call without checking the actual instruction. A longer instruction would leave trailing bytes to be decoded as new instructions.
- **Correct approach:** At each resolved site, `DisasmSingleInstruction` must decode `MOV EAX, [mem]` with length at least five. A failed decode or mismatch reports the symbol and address and does not write the patch. Build a buffer with NOPs over the full decoded instruction length, place the five-byte relative call at the start, then write exactly that length.
- **Verification:** Renderer Debug|Win32 MSBuild succeeded. A local Capstone probe confirmed that 5-byte `A1` and 6-byte `8B 05` forms meet the predicate; `MOV EAX, imm`, `MOV [mem], EAX`, and a two-byte `MOV EAX, EAX` do not. No in-game engine test was run.
- **Scope:** The seven gamedata site names, official-allocation guard, and texture generation target are unchanged.

## Studio API locators merged into `Engine_FillAddress` (2026-09-25)

- **Trigger / root cause:** `EngineStudio_FillAddress` (`exportfuncs.cpp`) still recovered six engine globals by disassembling the studio API thunks reached through `pstudio` (`GetTimes`, `SetForceFaceFlags`, `StudioSetRemapColors`, `StudioSetupModel`, `StudioSetupLighting`). Worse, `EngineStudio_FillAddress_GetTimes` re-derived `r_framecount` by scanning `GetTimes`, overwriting the value `Engine_FillAddress_R_RecursiveWorldNodeVars` had already resolved from gamedata; because `Sig_VarNotFound` only fires on a null pointer, the scan could only replace a correct address, never repair a missing one. The accompanying comment ("r_framecount remains catalog-uncovered") was stale — the same plugin had been resolving that name from gamedata since the 2026-09-17 migration.
- **Coverage check:** All twelve symbols are published as engine GLOBAL records on **11/11** engine identities (`cof-5936`, `hl-10210`, `hl-3248/3266/3329/3647/4554/6153/8684`, `svencoop-10257/8948`).
- **Implementation:** `EngineStudio_FillAddress` and its six locators are deleted from `exportfuncs.cpp`; their outputs move to seven new single-argument locators in `gl_hooks.cpp`, dispatched from `Engine_FillAddress` next to `Engine_FillAddress_R_StudioLighting`: `Engine_FillAddress_StudioGlobals` (`currententity`, `pstudiohdr`, `r_origin`), `_StudioGetTimes` (`cl_time`, `cl_oldtime`), `_StudioSetForceFaceFlags` (`g_ForcedFaceFlags`), `_StudioSetRemapColors` (`r_topcolor`, `r_bottomcolor`), `_StudioSetRenderamt` (`CL_FxBlend` FUNCTION), `_StudioSetupModel` (`psubmodel`), `_StudioSetupLighting` (`r_colormix`). Because `pstudio` was only ever a disasm anchor, the merged function needs no `engine_studio_api_s*`, and the two call sites already passed identical `(mirror-or-engine, engine)` arguments, so resolution simply moves from `HUD_GetStudioModelInterface` to `LoadEngine`. That is strictly earlier and behaviour-neutral: every output is a static data-section address assignment, and consumers only dereference them during rendering.
- **Dead-code cleanup:** `pbodypart` was written by `EngineStudio_FillAddress_StudioSetupModel` but had **zero readers** anywhere in the plugin (only a definition in `gl_studio.cpp`, an `extern` in `gl_studio.h`, and the locator's own write) — deleted rather than migrated, per the standing "upstream publishes ≠ consumer needs" rule, and deliberately kept out of the release gate. The `r_framecount` disassembly block, its `GetTimes_SearchContext`, and the stale comment are gone with it. `#include <capstone.h>` is dropped from `exportfuncs.cpp`, which no longer disassembles anything.
- **Gate / tests:** `RENDERER_ENGINE_ALL_GLOBALS` gains `g_ForcedFaceFlags`, `psubmodel`, `r_bottomcolor`, `r_colormix`, `r_topcolor`. New contract test `test_gate_requires_direct_resolved_studio_globals_on_every_identity` asserts each is required on every identity and that `pbodypart` never enters the gate.
- **Verification:** `MSBuild.exe MetaHook.sln -t:Plugins\Renderer -p:Configuration=Release -p:Platform=Win32` exited 0 (`Renderer.dll` produced; the nine emitted warnings are pre-existing and in untouched files/lines). `python -m pytest scripts/tests/test_gamedata_contract.py -q` passed 72 tests. `python scripts/validate-gamedata.py Build/svencoop/metahook/gamedata` reports the same 110 `virtualFunction` errors as the unmodified `HEAD` validator — a pre-existing mismatch between the locally cached packaged snapshots and the current vgui2 vfunc gate, unrelated to this change; none of the 110 mentions any of the five newly gated names. In-game studio rendering was **not** exercised.
- **Scope:** `Plugins/Renderer/exportfuncs.cpp`, `gl_hooks.cpp`, `gl_studio.cpp`, `gl_studio.h`, `scripts/validate-gamedata.py`, `scripts/tests/test_gamedata_contract.py`. The separate `Plugins/StudioEvents` plugin has its own unrelated `EngineStudio_FillAddress` and was not touched.

## EngineSurface vertex-buffer globals migrated to gamedata (2026-09-26)

- **Trigger / constraint:** `Engine_FillAddress_EngineSurface_drawFlushText` (`EngineSurfaceHook.cpp`) located `g_VertexBuffer` and `g_iVertexBufferEntriesUsed` by walking the mirror-engine `EngineSurface::drawFlushText` body (`DisasmRanges` `+0x150`) with four instruction heuristics — a `CMP [abs],imm` hit, a `MOV reg,[abs]`-then-`CMP reg,imm` two-instruction candidate pair, and a following `PUSH imm32(.data)` for the array address. Both names are published upstream as engine GLOBAL records, so the pass was redundant.
- **Coverage check:** Both `g_VertexBuffer` and `g_iVertexBufferEntriesUsed` exist as `kind: global`, `module: engine`, `platform: windows` records on **11/11** engine identities (`cof-5936`, `hl-10210`, `hl-3248/3266/3329/3647/4554/6153/8684`, `svencoop-10257/8948`) — the exact `RENDERER_ALL_GAMES` set. Their `gv_sig` payload is the same `drawFlushText` body the scan walked (e.g. `hl-3248` `A1 ?? ?? ?? ?? 56 83 F8 01 …`), so catalog and scan express the same fact.
- **Live consumers:** both are read by the `enginesurface_drawFlushText` VFTHook wrapper (installed at vtable index 22 for GoldSrc/SvEngine, 23 for HL25): `(*g_iVertexBufferEntriesUsed) > 0` gates the draw, `(*g_VertexBuffer)[i].texcoords` / `.vertex` feed the vertex list, and the counter is reset to 0 after `R_DrawTexturedRect`. Neither is write-only, so both stay in the gate.
- **Implementation:** the locator body shrinks from ~96 lines to two `GamedataResolvePtr(RealDllInfo.ImageBase, …, MH_GAMESYMBOL_KIND_GLOBAL)` assigns; the `SearchContext`, the `DisasmRanges` lambda, the `_VA` alias and the two `Sig_NotFound`/`Sig_VarNotFound` calls are deleted. Its signature drops the now-unused `DllInfo` parameter (`Engine_FillAddress_EngineSurface_drawFlushText(const mh_dll_info_t& RealDllInfo)`) to match the other single-argument locators; the sole call site is updated.
- **Dead-code cleanup:** `gPrivateFuncs.enginesurface_drawFlushText` had exactly one reader — the `ConvertDllInfoSpace` disasm root being removed — so the `private_funcs_t` field and both `GetVFunctionFromVFTable` assignments (HL25 and GoldSrc/SvEngine branches) are deleted, per the standing "upstream publishes ≠ consumer needs" rule. `index_enginesurface_drawFlushText` is kept: `EngineSurface_InstallHooks` still needs the vtable index for `VFTHook`. `EngineSurfaceHook.cpp` keeps `ConvertDllInfoSpace` and Capstone — `Engine_FillAddress_EngineSurface_pushMakeCurrent` still disassembles.
- **Gate / tests:** `RENDERER_ENGINE_ALL_GLOBALS` gains `g_VertexBuffer` and `g_iVertexBufferEntriesUsed`. New contract test `test_gate_requires_enginesurface_vertexbuffer_globals_on_every_identity` removes each name and asserts the gate reports it on every identity. The catalog FUNCTION record `EngineSurface::drawFlushText()` is deliberately **not** gated — the plugin obtains that function by hardcoded vtable index via `VFTHook`, not by name.
- **Verification:** `MSBuild.exe MetaHook.sln -target:Plugins\Renderer -p:Configuration=Release -p:Platform=Win32` exited 0 and rebuilt `intermediate/x86/Release/Renderer/EngineSurfaceHook.obj` (14:03); the emitted warnings are all pre-existing and in untouched files/lines. `python -m unittest test_gamedata_contract` from `scripts/tests` passed 73 tests (was 72). `python scripts/validate-gamedata.py Build/svencoop/metahook/gamedata` yields an error set **byte-identical** to the unmodified `HEAD` validator (235 lines, 235 errors) — the locally cached packaged snapshots are ahead of the last recorded baseline, so the count differs from the 110 noted on 2026-09-25, but both runs are the same pre-existing vgui2 `virtualFunction` ingestion failures, and no error mentions either newly gated name. In-game VGUI2 surface rendering was **not** exercised.
- **Scope:** `Plugins/Renderer/EngineSurfaceHook.cpp`, `Plugins/Renderer/privatehook.h`, `scripts/validate-gamedata.py`, `scripts/tests/test_gamedata_contract.py`, plus this note.

## Dead-code sweep (2026-09-26): 34 sig macros, 16 orphan hook handles, 18 write-only captures

- **Trigger / constraint:** `gl_hooks.cpp` was audited for symbols that are *semantically* unused, i.e. not merely unreferenced textually but unreachable or write-only. Three classes fell out. The distinction matters because two of them (`g_phook_*` handles, `gPrivateFuncs` captures) are *always* "referenced" somewhere — by a token-pasting macro or by the capture statement itself — so a plain grep for the identifier reports them as used.
- **Class 1 — 34 signature macros, all dead.** Every `*_SIG_*` / `*_BLOB` `#define` at the top of the file (`R_DRAWPARTICLES_SIG_BLOB`, `S_EXTRAUPDATE_*`, `R_DECALMPOLY_SIG*`, `R_DRAWDECALS_SIG*`, `R_RENDERVIEW_SIG_BLOB`, `V_RENDERVIEW_SIG_BLOB`, `R_FORCECVARS_SIG*`, `R_GLSTUDIODRAWPOINTS_SIG_BLOB2`, `BUILDNORMALINDEXTABLE_SIG*`, `MOD_LOADSPRITEMODEL_BLOB`, `R_INITPARTICLETEXTURE_*`, `DRAWSTARTUPGRAPHIC_BLOB`, `DRAW_SPRITEFRAME*`, `DRAW_FILLEDRGBA*`, `D_FILLRECT_BLOB`, `DRAW_PIC_BLOB`) dates from the pre-gamedata signature locators. After the migration every locator resolves through `GamedataResolvePtr`, so nothing expands them: each name occurs exactly once in the repo, on its own `#define` line.
- **Class 2 — 16 orphan `g_phook_*` handles (65 declared → 49).** `Install_InlineHook(X)` expands to `g_phook_##X` and `Uninstall_Hook(X)` to `g_phook_##X`, and every handle is file-`static`. A handle is therefore reachable *only* by passing its bare name to one of those two macros — the literal `g_phook_<name>` never appears a second time. The 16 below never do, so they are permanently `NULL` and no hook is ever recorded or removed through them: the twelve `triapi_*` handles (the pTriAPI hooks are installed by assigning `gEngfuncs.pTriAPI-><slot>` directly, bypassing the hook machinery entirely), `Mod_LoadBrushModel`, `ClientPortalManager_ResetAll`, `DLL_SetModKey` (which has no `gPrivateFuncs` field at all), and `SDL_GL_SetAttribute` (that symbol comes from SDL2.dll via `GetProcAddress`, never an inline hook). Note that "field is live" and "handle is live" are independent: `gPrivateFuncs.triapi_Fog` is read while `g_phook_triapi_Fog` is not.
- **Class 3 — 18 write-only `gPrivateFuncs` captures.** Fields whose only occurrence anywhere in the repo is their capture line. Twelve are the triapi originals (`triapi_RenderMode`/`Begin`/`End`/`Color4f`/`Color4ub`/`TexCoord2f`/`Vertex3fv`/`Vertex3f`/`Brightness`/`Color4fRendermode`/`GetMatrix`/`BoxInPVS`) — the plugin's replacement callbacks never chain to the engine original, so the captured pointers were dropped on the floor. **Counter-example worth keeping in mind:** `triapi_Fog`, `triapi_FogParams` and `triapi_SpriteTexture` *are* read (`gl_rmain.cpp:1905`, `:1910`, `:1917`) and were left alone — the twelve and the three are visually identical captures on adjacent lines. The other six dead captures are `R_DrawParticles`, `R_DrawSequentialPoly`, `R_DrawSequentialPoly_HL25`, `R_DrawTEntitiesOnList`, `R_TextureAnimation` and `GL_Shutdown`.
- **Root cause of the six function captures:** the plugin supplies its own `R_DrawParticles` (`gl_rmain.cpp:1015`), `R_DrawTEntitiesOnList` (`:1947`) and `GL_Shutdown` (`:6043`) bodies — declared in `gl_local.h`, defined in `gl_rmain.cpp` — and those bodies never chain to the engine originals; for `R_DrawSequentialPoly` the plugin never intercepts the engine body at all, and its texture-animation logic reads `rtable` directly. Do not confuse the *field* with the *plugin callback* of the same name: the `GL_Shutdown` at `gl_hooks.cpp:1247` is the callback handed to `InlinePatchRedirectBranch` and is perfectly live, while `gPrivateFuncs.GL_Shutdown` was not.
- **Cascade:** `Engine_FillAddress_R_DrawTEntitiesOnList` resolved nothing but its one dead field, so the whole locator and its dispatch call are gone. Four other locators lose exactly one line and keep their live symbols: `_R_DrawParticles` (keeps `R_FreeDeadParticles`, `R_TracerDraw`, `R_BeamDrawList`, `active_particles`, `particletexture`), `_R_DrawSequentialPoly` (both branches were dead; keeps `lightmaps`, `gDecalSurfCount`, and its now-single-line body no longer needs the `_VA` alias or the HL25 branch), `_R_TextureAnimation` (keeps `rtable`), `_GL_Shutdown` (keeps `Sys_ShutdownGame_to_GL_Shutdown_callsite_0`).
- **Deliberately NOT removed — the five write-through globals.** `c_alias_polys`, `detTexSupported`, `gl_extensions`, `lightgammatable` and `texgammatable` have zero *reads* inside the plugin, so a read/write scan flags them. They must stay: their only in-plugin references are dereferenced writes into engine memory (`(*c_alias_polys) = 0` at `gl_rmain.cpp:3118`, `(*gl_extensions) = ""` at `:5969`, `(*detTexSupported) = false` at `:6011`, the identity-table fills at `gl_draw.cpp:3535`/`:3541`), and the engine reads what the plugin wrote. Write-through is a consumer, not a dead store — this is the mirror image of the "upstream publishes ≠ consumer needs" rule.
- **Hook asymmetry fixed:** `Install_InlineHook(Host_ClearMemory)` had no matching uninstall, so the hook was never removed at exit. `Uninstall_Hook(Host_ClearMemory)` is inserted between `PVSNode` and `CVideoMode_Common_DrawStartupGraphic`, mirroring the install order; all 49 installs are now paired.
- **Hook-teardown convention found, and a missing call fixed.** `Client_UninstallHooks` was defined (`gl_hooks.cpp`) and declared (`privatehook.h`) but never called, so the four client hooks (`ClientPortalManager_DrawPortalSurface`, `EnableClipPlane`, `RenderPortals`, `UpdatePlayerPitch`) stayed installed past shutdown. The initial guess — that it belonged in `IPluginsV4::ExitGame` — is **wrong for client-side hooks**: no plugin in the repo calls it from `ExitGame`. The convention is the `HUD_Shutdown` export, because client hook lifetime ends with the client DLL, not the engine. `Plugins/VGUI2Extension/exportfuncs.cpp:147` and `Plugins/CaptionMod/exportfuncs.cpp:86` both call `Client_UninstallHooks()` at the top of `HUD_Shutdown` and then chain to `gExportfuncs.HUD_Shutdown()`. (`IPluginsV4::ExitGame` is where *engine*-side uninstalls go — see the Renderer's own `ExitGame` calling `Engine_UninstallHooks` / `EngineSurface_UninstallHooks` / `VideoMode_UninstallHooks`. Note also that `MH_ExitGame` calls `MH_FreeAllHook()` immediately after dispatching `ExitGame`, so an explicit uninstall there is largely redundant with the loader's central reclaim.) Fix: `Client_UninstallHooks()` is inserted in `Plugins/Renderer/exportfuncs.cpp` `HUD_Shutdown`, grouped with its sibling `ClientStudio_UninstallHooks()`. The Renderer's existing order is kept — it chains to `gExportfuncs.HUD_Shutdown()` *first* and uninstalls afterwards — rather than adopting VGUI2Extension/CaptionMod's uninstall-first order, because reordering the three pre-existing `*_UninstallHooks` calls would be a behaviour change beyond the missing line.
- **Same gap fixed in `Plugins/SCCameraFix` (no `HUD_Shutdown` existed there).** SCCameraFix had the identical defect: `Client_UninstallHooks` defined (`privatehook.cpp:274`), declared (`privatehook.h:53`), never called; its only hook is the client-side `V_CalcNormalRefdef`. It had no `HUD_Shutdown` export at all (only `CAM_Think` / `HUD_Init` were wired in `LoadClient`) and both `IPluginsV4::ExitGame` and `Shutdown` were empty, so the convention-preserving fix adds one: declaration in `exportfuncs.h`, definition in `exportfuncs.cpp` (`Client_UninstallHooks(); gExportfuncs.HUD_Shutdown();`), and `pExportFunc->HUD_Shutdown = HUD_Shutdown;` in `LoadClient`. This widens the plugin's engine-facing export surface, which is why it was confirmed with the user rather than assumed. Left untouched in that plugin: `Engine_InstallHooks` / `Engine_UninstallHooks` are declared at `privatehook.h:31-32` but have **no definition and no caller** — template scaffolding, reported not removed. `Plugins/DontFlushSoundCache/privatehook.h:32` likewise declares `Client_UninstallHooks` with no definition and no call.
- **Includes:** `<utlvector.h>` and `<SDL2/SDL_video.h>` are dropped from `gl_hooks.cpp`; the file uses no `CUtl*` or `SDL_*` type (the SDL pointers in `privatehook.h` are typed `void*`). `<capstone.h>` (`cs_insn`, `X86_INS_MOV`) and `<cstring>` (`strcmp`, `std::memset`) are still needed and stay.
- **Gate / tests:** `RENDERER_ENGINE_ALL_FUNCTIONS` loses the five names the plugin no longer resolves (`GL_Shutdown`, `R_DrawParticles`, `R_DrawSequentialPoly`, `R_DrawTEntitiesOnList`, `R_TextureAnimation`); gating them would make the release depend on records with no consumer. `test_gate_requires_lightmap_and_decal_symbols_on_every_identity` drops `R_TextureAnimation` from its required list (its seven globals are unaffected), and the new `test_gate_ignores_function_resolutions_the_renderer_never_read` asserts all five stay out of every gate tuple.
- **Verification:** `MSBuild.exe MetaHook.sln -target:Plugins\Renderer -p:Configuration=Release -p:Platform=Win32` exited 0 and rebuilt `intermediate/x86/Release/Renderer/gl_hooks.obj`; the emitted warnings are pre-existing and in untouched files/lines. After the `HUD_Shutdown` line was added, `Plugins\Renderer` and `Plugins\SCCameraFix` were each built again for Release|Win32 — both exited 0 with no errors, producing `output/x86/Release/Renderer.dll` and `SCCameraFix.dll`. `python -m unittest test_gamedata_contract` from `scripts/tests` passed 74 tests (was 73). `python scripts/validate-gamedata.py Build/svencoop/metahook/gamedata` produces an error set **byte-identical** to the unmodified `HEAD` validator (235 lines / 235 errors) — the same pre-existing vgui2 `virtualFunction` ingestion failures recorded on 2026-09-25/26, so this change adds zero errors and no error mentions any removed name. In-game rendering and shutdown were **not** exercised, so the `HUD_Shutdown` wiring is verified only as far as compilation.
- **Scope:** `Plugins/Renderer/gl_hooks.cpp`, `Plugins/Renderer/privatehook.h`, `Plugins/Renderer/exportfuncs.cpp`, `Plugins/SCCameraFix/plugins.cpp`, `Plugins/SCCameraFix/exportfuncs.h`, `Plugins/SCCameraFix/exportfuncs.cpp`, `scripts/validate-gamedata.py`, `scripts/tests/test_gamedata_contract.py`, plus this note.

## EngineSurface window/scissor globals migrated to gamedata, and the same locator in VGUI2Extension (2026-09-26)

- **Trigger / constraint:** `Engine_FillAddress_EngineSurface_pushMakeCurrent` (`Plugins/Renderer/EngineSurfaceHook.cpp`) was that file's last disasm user: it walked the mirror engine's `EngineSurface::pushMakeCurrent` body (`DisasmRanges` `+0x500`) with three heuristics to find `pmainwindow` (`MOV reg,[.data]` within the first 35 instructions, validated by `MOV reg,[candReg]` / `PUSH [candReg]` within `+6`), `g_bScissor` (`MOV [.data],1`) and `g_ScissorRect` (up to 4 successive `MOV [.data],reg`, `qsort`, lowest). All three are published upstream as engine GLOBAL records, so the pass was redundant.
- **Coverage check:** all three exist as `kind: global`, `module: engine`, `platform: windows` on **11/11** identities — exactly one windows record each, and exactly the `RENDERER_ALL_GAMES` set. Stronger: the catalog holds **11 distinct engine-module CRC64 values in total**, and every one of them carries all three names, so no engine binary the loader can hash is left uncovered. That matters because `MH_QueryGameSymbol` keys on `(module CRC64, symbolName)` alone (`src/GameData.cpp:1306-1313`), never on gameVersion — which is how a cstrike / czero / czeroR install, whose snapshots publish no engine module at all, still resolves its engine records out of the HL snapshot.
- **Record shape corroborates the heuristics:** each payload's reference instruction matches its heuristic exactly — `pmainwindow` is `gv_inst_length 5` / `gv_inst_disp 1` (`A1 <disp32>` = `MOV reg,[.data]`), `g_bScissor` is `7` / `2` (`C7 05 <disp32> 01` = `MOV [.data],1`), `g_ScissorRect` is `6` / `2` (`89 0D <disp32>` = `MOV [.data],reg`). The RVAs also agree with the `qsort`-lowest rule: `g_ScissorRect` sits `0x18` below `g_bScissor` on every identity (e.g. `hl-4554` `0x74deb8` vs `0x74ded0`).
- **Live consumers (all three therefore stay in the gate):** `pmainwindow` is read by `Sys_GetMainWindow()` (`gl_rmain.cpp:428`, `**pmainwindow`); `g_bScissor` gates `ScissorRect_TCoords` (`EngineSurfaceHook.cpp:72`); `g_ScissorRect` is intersected there (`:74`) and filled by the `pushMakeCurrent` wrapper (`:207-210`).
- **Implementation:** the locator shrinks from ~118 lines to three `GamedataResolvePtr(RealDllInfo.ImageBase, …, MH_GAMESYMBOL_KIND_GLOBAL)` assigns; the `SearchContext`, the `DisasmRanges` lambda, the `_VA` alias / `ConvertDllInfoSpace` call, the `std::qsort` block and the three `Sig_VarNotFound` calls are deleted. Its signature drops the now-unused `DllInfo` parameter to match the sibling `Engine_FillAddress_EngineSurface_drawFlushText`, and the sole call site is updated. `<capstone.h>` and `<cstdlib>` lose their last use in this file and are dropped.
- **Cascade:** `gPrivateFuncs.enginesurface_pushMakeCurrent`'s only reader was that disasm root, so the field (`privatehook.h`) and its two `GetVFunctionFromVFTable` assignments go away; `index_enginesurface_pushMakeCurrent` stays because the `VFTHook` at index 1 needs it. Same shape as the `enginesurface_drawFlushText` field deleted earlier the same day.
- **Same locator, second copy, migrated too:** `Plugins/VGUI2Extension/EngineSurfaceHook.cpp` carried an almost identical `EngineSurface_FillAddress_pushMakeCurrent` (only difference: it checked `Sig_VarNotFound(pmainwindow)` alone). It is migrated the same way, `<capstone.h>` / `<cstdlib>` are dropped, and **two** fields die with it: `gPrivateFuncs.enginesurface_pushMakeCurrent` (only reader was the disasm root) and `index_enginesurface_pushMakeCurrent` (only reader was that branch's single `GetVFunctionFromVFTable` call — this plugin's `EngineSurface_InstallHooks` is a no-op, so slot 1 was never hooked). The two `engineSurface_vftable` locals existed only to feed that call and go too, leaving `EngineSurface_FillAddress` to create the surface and resolve the three globals; its `DllInfo` parameter is kept because the plugin's top-level passes (`Engine_FillAddress`, `Client_FillAddress`, `Engine_PatchAddress_*`) share that shape. VGUI2Extension had **no gamedata dependency at all** before this, so `Plugins/VGUI2Extension/plugins.h` gains `#include <metahook.h>`, a `static_assert(METAHOOK_API_VERSION >= 109)` and its own `GamedataResolvePtr` helper (GLOBAL kind only — no `...IfAvailable` variant, because all three records are mandatory on every identity).
- **Gate / tests:** `g_ScissorRect`, `g_bScissor` and `pmainwindow` are added to `RENDERER_ENGINE_ALL_GLOBALS`, and `test_gate_requires_the_engine_surface_scissor_and_window_globals` asserts each is required on every identity. The tuple is named `RENDERER_*` but covers VGUI2Extension transitively: both plugins resolve against the same engine module CRCs, and the gate pins the three names on all 11 identities — shown above to be the complete set of engine CRCs in the catalog.
- **Verification:** `MSBuild.exe MetaHook.sln "-target:Plugins\Renderer;Plugins\VGUI2Extension" -p:Configuration=Release -p:Platform=Win32` exited 0 with both `EngineSurfaceHook.cpp` files and `VGUI2Extension/privatefuncs.h` `touch`ed first to force a real recompile — the log shows `EngineSurfaceHook.cpp` recompiled in both projects and `privatefuncs.cpp` in VGUI2Extension, producing `output/x86/Release/Renderer.dll` and `VGUI2Extension.dll` with no errors and no new warnings. `python -m unittest test_gamedata_contract` from `scripts/tests` passes **75** tests (was 74). `python scripts/validate-gamedata.py Build/svencoop/metahook/gamedata` reports 334 errors, **byte-identical** to the `HEAD` validator's set (334) — the same pre-existing vgui2 `virtualFunction` ingestion failures recorded on 2026-09-25/26, none mentioning a scissor/window name — so this change adds zero errors. In-game scissor clipping and the main-window path were **not** exercised; resolution is verified only as far as compilation plus the catalog contract.
- **Scope:** `Plugins/Renderer/EngineSurfaceHook.cpp`, `Plugins/Renderer/privatehook.h`, `Plugins/VGUI2Extension/EngineSurfaceHook.cpp`, `Plugins/VGUI2Extension/privatefuncs.h`, `Plugins/VGUI2Extension/plugins.h`, `scripts/validate-gamedata.py`, `scripts/tests/test_gamedata_contract.py`, plus this note.
