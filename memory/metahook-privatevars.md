...
> **gamedata 迁移状态（2026-08-30）**：以下 10 个「最终消费」符号已改为由本地 gamedata catalog 经 `ResolveGameSymbol` 解析，不再使用镜像映像 + 签名/字符串/交叉引用扫描：
...
> `ResolveGameSymbol` 始终返回 `moduleBase + rva`。`cl_enginefuncs` 和 `cl_funcs` 的 RVA 都表示内嵌 global object 本身；只有拦截 `ClientDLL_Init` 对 `cl_funcs.pInitFunc` 的间接调用时，才由 `MH_LoadEngine_ResolveGlobalOperand` 使用该记录的 `signatureRva + instructionOffset + operandOffset` 计算机器指令 operand 地址。
>
...
| `ClientDLL_Init`（引擎内 client DLL 初始化例程；不保存为本地函数指针） | gamedata 的 `cl_funcs` global metadata | `cl_funcs.signatureRva` 指向该函数签名，`instructionOffset + operandOffset` 指向 `call [cl_funcs.pInitFunc]` 的 absolute operand。 | `MH_LoadEngine_ResolveGlobalOperand` 仅据此取得调用 operand 并验证它仍引用已解析的 `cl_funcs`；不改变 `ResolveGameSymbol` 的 direct-global 合约。 |
...
| --- | --- | --- | --- |
| `g_pEngineFuncs`（游戏侧正式对象 `cl_enginefuncs`） | `src/metahook.cpp` | `ResolveGameSymbol("cl_enginefuncs")` 直接返回内嵌 `cl_enginefunc_t` global object 的地址，不再二次解引用。 | 复制到 `gMetaSave.pEngineFuncs`，并直接传给 V3/V4 插件的 `LoadEngine`。 |
| `g_pExportFuncs`（游戏侧正式对象 `cl_funcs`） | `src/metahook.cpp` | `ResolveGameSymbol("cl_funcs")` 直接返回内嵌 client export table 的地址；其 ABI 布局与 `cl_exportfuncs_t` 对应。 | `ClientDLL_Initialize` 在引擎填表后复制、传给插件并调用原 `Initialize`；初始化接管写入的是另行计算的调用 operand，不是 `cl_funcs` global 首地址。 |
| `g_pClientDLLInitializeOperand`（`call [cl_funcs.pInitFunc]` 的 absolute operand） | `src/metahook.cpp` | `MH_LoadEngine_ResolveGlobalOperand` 按 `moduleBase + signatureRva + instructionOffset + operandOffset` 计算，并验证 operand 当前指向 direct-global `cl_funcs`。 | `MH_WriteDWORD` 将该 operand 改指向 MetaHook 的 wrapper slot；这是为保持 client 加载后填表、随即调用的原始时序。 |
...
- `g_pExportFuncs` 直接指向 `cl_funcs`，不是从机器指令 operand 二次解引用得到；`ClientDLL_Initialize` 在引擎完成 client export 填表后复制、传给插件并调用原 `Initialize`。
- `gMetaSave.pEngineFuncs` 从 direct-global `cl_enginefuncs` 复制，随后作为公开 `cl_enginefunc_t` 表使用。因此 `Cmd_GetCmdBase` 仍不属于独立定位的私有函数。
