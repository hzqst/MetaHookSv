...
call cmake -G "Visual Studio 17 2022" -S "%SolutionDir%thirdparty\curl" -B "%SolutionDir%thirdparty\build\libcurl\x86\Release" -A Win32 -DCMAKE_INSTALL_PREFIX="%SolutionDir%thirdparty\install\libcurl\x86\Release" -DCMAKE_TOOLCHAIN_FILE="%SolutionDir%tools\toolchain.cmake" -DBUILD_CURL_EXE=FALSE -DBUILD_EXAMPLES=FALSE -DBUILD_TESTING=FALSE -DBUILD_LIBCURL_DOCS=FALSE -DBUILD_MISC_DOCS=FALSE -DCURL_ZLIB=FALSE -DCURL_ZSTD=FALSE -DCURL_BROTLI=FALSE -DENABLE_CURL_MANUAL=FALSE -DENABLE_THREADED_RESOLVER=FALSE -DPICKY_COMPILER=FALSE -DUSE_LIBIDN2=FALSE -DUSE_NGHTTP2=FALSE -DUSE_WIN32_IDN=TRUE -DCURL_USE_LIBPSL=FALSE -DUSE_LIBPSL=FALSE -DCURL_USE_LIBSSH2=FALSE -DUSE_LIBSSH2=FALSE -DCURL_USE_SCHANNEL=TRUE -DCURL_ENABLE_SSL=FALSE -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DUSE_VCLTL=TRUE

call cmake --build "%SolutionDir%thirdparty\build\libcurl\x86\Release" --config Release --target install