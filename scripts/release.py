...
def validate_codex_events(output):
    for line in output.splitlines():
        ...
        item = event.get("item", {})
        if (item.get("type") == "mcp_tool_call" and item.get("server") == "release_git"
...
def run_cli_once(context, provider, model, endpoint, key, repository=None):
    ...
    with tempfile.TemporaryDirectory(prefix="release-ai-", dir=os.environ.get("RUNNER_TEMP")) as temporary:
        ...
        work.mkdir()
        home.mkdir()
        git_server = {"command": sys.executable,
                      "args": ["-B", str(Path(__file__).with_name("release_git.py").resolve()),
                               "--repository", str(repository),
                               "--budget", str(MAX_CONTEXT_BYTES - len(context.encode("utf-8")))]}
        ...
        command = ["claude", "--print", "--model", model, "--output-format", "json",
                   "--tools", "", "--allowedTools", "mcp__release_git__git_history",
