...
NTSTATUS NTAPI LdrLoadDllMemoryExW(
	...
	__try
	{
		...
		if (!RtlIsValidImageBuffer(BufferAddress, BufferSize, OutImageSize) && !(dwFlags & LOAD_FLAGS_PASS_IMAGE_CHECK))
	...
	if (DllName)
	{
		...
		while (ListEntry != ListHead)
		{
			...
			if (!_wcsnicmp(DllName, CurEntry->BaseDllName.Buffer, (CurEntry->BaseDllName.Length / sizeof(wchar_t)) - 4) ||
				...
				if (!(h2 = RtlImageNtHeader(CurEntry->DllBase)))
					continue;

				if (!(module = MapMemoryModuleHandle((HMEMORYMODULE)CurEntry->DllBase)))
					continue;
	...
	if (headers->OptionalHeader.DllCharacteristics & IMAGE_DLLCHARACTERISTICS_NO_SEH)
	{
	...
	if (dwFlags & LOAD_FLAGS_NOT_MAP_DLL)
	{
		do {
			...
			if (!(dwFlags & LOAD_FLAGS_NO_RESOLVE_IMPORTS))
			{
			...
			status = MemorySetSectionProtection(LPBYTE(*BaseAddress), headers, dwFlags);
	...
	do
	{
		...
		if (!(dwFlags & LOAD_FLAGS_NOT_USE_REFERENCE_COUNT))
		{
			module->UseReferenceCount = true;
		}
		...
		if (!(dwFlags & LOAD_FLAGS_NOT_HANDLE_TLS) && MmpGlobalDataPtr->MmpFunctions->_MmpHandleTlsData)
		{
...
NTSTATUS NTAPI LdrUnloadDllMemory(_In_ HMEMORYMODULE BaseAddress) {
	...
	do
	{
		...
		if (module->MappedDll) {
			...
			if (module->TlsHandled && MmpGlobalDataPtr->MmpFunctions->_MmpReleaseTlsEntry) {
