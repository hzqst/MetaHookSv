...
NTSTATUS InitializeLockHeld() {
    ...
    do {
		...
		MmpGlobalDataPtr->MmpFunctions->_MmpHandleTlsData = NULL;
		MmpGlobalDataPtr->MmpFunctions->_MmpReleaseTlsEntry = NULL;
...
extern "C" __declspec(dllexport) BOOL WINAPI ReflectiveMapDll(HMODULE hModule) {
	...
	NTSTATUS status = MmpInitializeStructure(0, 0, nullptr, headers);
