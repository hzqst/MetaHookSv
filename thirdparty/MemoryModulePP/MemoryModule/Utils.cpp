...
BOOLEAN NTAPI RtlIsValidImageBuffer(
	...
	__try {
		...
		case IMAGE_NT_OPTIONAL_HDR32_MAGIC:
			pSectionHeader = PIMAGE_SECTION_HEADER((char*)&headers.nt32->OptionalHeader + headers.nt32->FileHeader.SizeOfOptionalHeader);
			SizeofImage = headers.nt32->OptionalHeader.SizeOfHeaders;

			if ((PUCHAR)pSectionHeader + headers.nt32->FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER) > (PUCHAR)Buffer + BufferSize)
				return FALSE;

			ProbeForRead(pSectionHeader, headers.nt32->FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER));
			
			for (WORD i = 0; i < headers.nt32->FileHeader.NumberOfSections; ++i, ++pSectionHeader)
				SizeofImage += pSectionHeader->SizeOfRawData;
		...
		case IMAGE_NT_OPTIONAL_HDR64_MAGIC:
			pSectionHeader = PIMAGE_SECTION_HEADER((char*)&headers.nt64->OptionalHeader + headers.nt64->FileHeader.SizeOfOptionalHeader);
			SizeofImage = headers.nt64->OptionalHeader.SizeOfHeaders;

			if ((PUCHAR)pSectionHeader + headers.nt32->FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER) > (PUCHAR)Buffer + BufferSize)
				return FALSE;

			ProbeForRead(pSectionHeader, headers.nt64->FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER));

			for (WORD i = 0; i < headers.nt64->FileHeader.NumberOfSections; ++i, ++pSectionHeader)
				SizeofImage += pSectionHeader->SizeOfRawData;
		...
		if (SizeofImage > BufferSize)
			return FALSE;

		ProbeForRead(Buffer, SizeofImage);

		if (OutImageSize) {
			*OutImageSize = SizeofImage;
		}

		if (!CheckSum)
			return TRUE;

		result = CheckSumBufferedFile(Buffer, SizeofImage, CheckSum);
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		SetLastError(RtlNtStatusToDosError(GetExceptionCode()));
	}
